...
FastAPI project. **Phase 1 (real crawler) is implemented and tested** —
see `app/crawler/`. The API layer (`app/api/`), the intelligence engines
(`app/engines/`), and everything past the crawl layer are still empty
...
- `app/crawler/` — the two-tier crawler (Crawlee `BeautifulSoupCrawler` by
  ...
  discovery, URL/domain normalization, page fingerprinting, and page/link
  extractors. See `../docs/CRAWLER.md` for the design.
...
- `app/tests/` — 27 tests, all real: unit tests for normalization/
...
- Page-level extraction covers the deterministic Phase 1 field set
  (title, meta description, canonical, headings, word count, language,
  robots meta). schema.org / OpenGraph / Twitter-card / entity extraction
  is Phase 2.
